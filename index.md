...
The central control application will serve two main purposes: provide a graphical interface that allows users to set system configurations, and run back end logic that controls the hardware. After loading any previously saved configurations, the D.R.E.W. System Tool starts background processes and shows the GUI to the user. The user can monitor the state of the system, as well as make changes to the current configuration. These configs will be saved when exiting the program. The logic that drives the system can be broken down into two main parts: the <b>Serial Reader</b>, and the <b>Bluetooth Controller</b>. Here was the preliminary <a href="https://github.com/danhipke/danhipke.github.io/raw/master/images/storyboard.pdf" target="_blank">UI storyboard</a>.

<b>Serial Reader:</b>
The Serial Reader is responsible for the initial reading and processing of messages from the hardware. It is connected over serial to the USB module, and constantly reads any messages that the USB module receives. It then proceeds to process the message, and attempts to determine if the current system state has changed. If any actions need to be taken, it puts a work item into a queue that tells the bluetooth controller which actions to take.
...
The bluetooth controller reads items from a work queue, each of which contains information regarding the relavent zone and entry/exit action. The controller then finds all bluetooth devices in the zone, and attempts to update their state the new desired one. It will attempt to connect to disconnected devices, but will only do so a few times to avoid hanging the system.
